...
def is_similar_video(a: dict, b: dict, duration_tol: int = 2, size_tol: float = 0.15) -> bool:
    """
    مقارنة فيديوين بناءً على المدة والحجم.
    duration_tol : فرق مقبول بالثواني (افتراضي ±2)
    size_tol     : نسبة فرق مقبولة في الحجم (افتراضي 15%)
    """
    if a["type"] != b["type"] or a["type"] != "video":
        return False
...
class Database:
    ...
    def stream_duplicates(self, channel_id, keep_strategy, min_size=0,
        ...
        # ── Layer 4: Fuzzy Video Matching (مدة + حجم) ──
        if use_fuzzy:
            videos = self.get_all_videos(channel_id, min_size)
            # نرتبهم حسب الاستراتيجية
            ...
            matched: set = set()  # file_ids اللي تم إيجاد مجموعة لها

            for i, a in enumerate(videos):
                if a["file_id"] in matched or a["id"] in seen_msg_ids:
                    continue
                group_ids = [a["id"]]
                for j, b in enumerate(videos):
                    if i == j: continue
                    if b["id"] in seen_msg_ids: continue
                    if a["file_id"] == b["file_id"]: continue  # هاد Layer 1 يغطيه
                    if is_similar_video(a, b, fuzzy_duration_tol, fuzzy_size_tol):
                        group_ids.append(b["id"])
                        matched.add(b["file_id"])

                if len(group_ids) > 1:
                    matched.add(a["file_id"])
